struct GradientVertexInput {
    ...
    @location(5) radial_data: vec4<f32>, // center.xy, inner_radius, outer_radius
...
struct GradientVertexOutput {
    ...
    @location(6) radial_data: vec4<f32>, // center.xy, inner_radius, outer_radius
...
fn gradient_vs_main(input: GradientVertexInput) -> GradientVertexOutput {
    ...
    // Scale the radial data appropriately
    out.radial_data = vec4<f32>(
        input.radial_data.xy * globals.scale, // center position
        input.radial_data.z * globals.scale,  // inner radius
        input.radial_data.w * globals.scale   // outer radius
    );
...
fn radial_gradient(
    ...
    radial_data: vec4<f32>, // center.xy, inner_radius, outer_radius
    colors: array<vec4<f32>, 8>,
    offsets: array<f32, 8>,
    last_index: i32
) -> vec4<f32> {
    let center = radial_data.xy;
    let inner_radius = radial_data.z;
    let outer_radius = radial_data.w;

    // Calculate distance from center
    let distance_from_center = length(raw_position - center);

    // Normalize the distance to a 0-1 range between inner and outer radius
    let coord_offset = clamp(
        (distance_from_center - inner_radius) / (outer_radius - inner_radius),
        0.0,
        1.0
    );

    // Store these as vars to use dynamic indexing in a loop
...
fn gradient_fs_main(input: GradientVertexOutput) -> @location(0) vec4<f32> {
    ...
    var mixed_color: vec4<f32> = radial_gradient(input.position.xy, input.radial_data, colors, offsets, last_index);
